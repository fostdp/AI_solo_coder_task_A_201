  ...
  return (
    <div className="flex h-full flex-col gap-5 p-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-2xl font-bold tracking-wide text-amber-200">缺陷预测分析</h1>
          <p className="mt-1 text-sm text-amber-200/60">基于 Niyama 判据的缩孔缩松预测</p>
        </div>
      </header>
      ...
      <div className="grid flex-1 grid-cols-5 gap-4 min-h-0">
        <div className="col-span-2 flex flex-col rounded-xl border border-amber-800/30 bg-[#0d0a07]/60 p-4 min-h-0">
          <h3 className="mb-3 text-sm font-semibold tracking-wide text-amber-300">Niyama 判据分布</h3>
          <p className="mb-3 text-[11px] leading-relaxed text-amber-400/60">
            Niyama = G / √R，其中 G 为温度梯度(°C/mm)，R 为冷却速率(°C/s)。
            当 Niyama &lt; 1.0 时，单元存在缩孔缩松风险。
          </p>
...
function MetricCard({
  ...
  icon: any; label: string; value: string; unit: string; color: string; highlight?: boolean;
}) {
  return (
    <div
    ...
    >
      ...
      <div className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full opacity-10 blur-3xl" style={{ backgroundColor: color }} />
    </div>
  );
}
